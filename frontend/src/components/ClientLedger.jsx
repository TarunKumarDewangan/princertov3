...
    // --- HELPER: GET LOCAL TIME (Pre-fill Input) ---
...
    // Open Add Modals (Pre-fill Date)
    const openAddWork = () => {
        setEditingItem(null);
        setWorkForm({
            job_date: getCurrentLocalTime(), // <--- Pre-fill Current Time
            vehicle_no: "",
            description: "",
            bill_amount: "",
            paid_amount: ""
...
        e.preventDefault();
        const payload = { ...workForm, client_id: id }; // Uses workForm.job_date which user can edit
        try {
...
        try {
            if (editingItem) {
                await api.put(`/api/work-jobs/${editingItem.id}`, { job_date: payForm.job_date, bill_amount: 0, paid_amount: payForm.amount, description: "PAYMENT RECEIVED", client_id: id });
            } else {
                await api.post('/api/work-jobs', {
                    client_id: id,
                    job_date: payForm.job_date, // Uses editable date
                    bill_amount: 0,
...
                                            value={workForm.job_date}
                                            onChange={e=>setWorkForm({...workForm, job_date:e.target.value})}
                                            required
                                        />
                                    </div>
...
                                        type="datetime-local"
                                        className="form-control"
                                        value={payForm.job_date}
                                        onChange={e=>setPayForm({...payForm, job_date:e.target.value})}
                                        required
                                    />
                                </div>
