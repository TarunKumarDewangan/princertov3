...
export default function ClientLedger() {
    ...
    // Forms
    const [workForm, setWorkForm] = useState({ job_date: getCurrentLocalTime(), vehicle_no: "", description: "", bill_amount: "", paid_amount: "" });
    const [payForm, setPayForm] = useState({ job_date: getCurrentLocalTime(), amount: "" });
    ...
    // UPDATED: Shows specific error message from server
    const handleSendReminder = async () => {
        ...
        try {
            ...
            // Extract error message (e.g. "No mobile linked", "No Dues", etc.)
            const msg = error.response?.data?.message || "Failed to send WhatsApp";
            toast.error(msg);
        } finally {
            setSendingMsg(false);
        }
    ...
    // Open Edit (Pre-fill DB Time)
    const openEdit = (item, type) => {
        setEditingItem(item);

        // Convert DB time to Input Format
    ...
    // --- SUBMIT WORK (TIME FIX) ---
    const handleWorkSubmit = async (e) => {
        e.preventDefault();

        // If editing, use form date. If new, use form date (which is pre-filled with current time)
        const payload = { ...workForm, client_id: id };

        try {
            if (editingItem) await api.put(`/api/work-jobs/${editingItem.id}`, payload);
            else await api.post('/api/work-jobs', payload);
            toast.success("Saved!"); setShowWorkModal(false); fetchData();
                    ...
                    <div className="d-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <small className="text-muted" style={{fontSize: '11px'}}>
                                {data.client.mobile ? data.client.mobile : <span className="text-danger">No Mobile</span>}
                            </small>
                        </div>
                    </div>
                    <div className="d-flex align-items-center gap-2">
                        ...
                        <div className="modal-content border-0 shadow-lg">
                            ...
                            <form onSubmit={handleWorkSubmit}>
                                <div className="modal-body p-3">
                                    {/* DATE INPUT */}
                                    <div className="mb-2">
                                        <label className="small fw-bold text-muted">Date & Time</label>
                                        <input type="datetime-local" className="form-control" value={workForm.job_date} onChange={e=>setWorkForm({...workForm, job_date:e.target.value})} required />
                                    </div>
                        ...
                        <div className="modal-content border-0 shadow-lg">
                            ...
                            <div className="modal-body p-4 text-center">
                                ...
                                <div className="mb-3 text-start">
                                    <label className="small fw-bold text-muted">Date & Time</label>
                                    <input type="datetime-local" className="form-control" value={payForm.job_date} onChange={e=>setPayForm({...payForm, job_date:e.target.value})} required />
                                </div>
