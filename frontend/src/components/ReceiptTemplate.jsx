...
export const ReceiptTemplate = React.forwardRef(({ data, selectedIds }, ref) => {
  ...
  // FIX: Filter using uniqueKey, not ID
  const itemsToPrint = data.statement.filter((item) => selectedIds.includes(item.uniqueKey));
  ...
  return (
    <div ref={ref} className="p-5" style={{ width: "100%", backgroundColor: "white", color: "black", fontFamily: "Arial, sans-serif" }}>
      ...
      <div className="d-flex justify-content-between border-bottom pb-3 mb-4">
        <div><h2 className="fw-bold text-primary m-0">RTO MANAGEMENT</h2><p className="text-muted small">Official Account Statement</p></div>
        <div className="text-end"><h4 className="fw-bold m-0">STATEMENT</h4><p className="small text-muted">Date: {new Date().toLocaleDateString("en-GB")}</p></div>
      </div>
      ...
      <table className="table table-bordered border-dark mb-0">
        <thead className="table-light"><tr><th>Date</th><th>Vehicle</th><th>Service</th><th className="text-end">Bill</th><th className="text-end">Paid</th><th className="text-end">Balance</th></tr></thead>
        <tbody>
        ...
        <div className="col-5">
          <table className="table table-sm table-borderless">
            <tbody>
              ...
              <tr className="border-top border-dark"><td className="text-end fs-5 fw-bold">Balance Due:</td><td className="text-end fs-5 fw-bold text-danger">₹{totalDue.toLocaleString()}</td></tr>
            </tbody>
          </table>
        </div>
      </div>
