...
    // Get Local Time for Input (YYYY-MM-DDTHH:MM)
...
    // Helper to format Date ONLY (Hide time in Date col)
    const formatEntryTime = (dateStr) => {
        if (!dateStr) return { date: '-', time: '' };
        const d = new Date(dateStr);
        return {
            date: d.toLocaleDateString('en-GB'),
            time: "" // Empty string = Hidden
        };
    };
    // Format System Time (Created At timestamp)
...
        accounts: [], entries: [],
        stats: { all: { in: 0, out: 0, balance: 0 }, daily: { in: 0, out: 0, balance: 0 } }
    });

    // Filters
    const [viewMode, setViewMode] = useState('all'); // Default 'all' so staff see data
...
        ledger_account_id: "",
        amount: "",
        entry_date: getCurrentLocalTime(), // Init with NOW
        description: ""
    });
...
        if (isFiltered) return data.entries;
        if (viewMode === 'daily') {
            const todayStr = new Date().toDateString();
            return data.entries.filter(e => new Date(e.entry_date).toDateString() === todayStr);
        }
        return data.entries;
    };
...
        ? { in: data.total_in || 0, out: data.total_out || 0, balance: data.balance || 0 }
...
            setData(prev => ({ ...prev, entries: res.data.entries, total_in: res.data.total_in, total_out: res.data.total_out, balance: res.data.balance }));
            setIsFiltered(true);
...
                            </div>
                        </div>
                        <div className="col-6 col-md-4"><div className="card border-0 shadow-sm p-3 bg-success-subtle h-100"><small className="text-success fw-bold">TOTAL IN</small><h4 className="fw-bold mb-0 text-success">₹{Number(currentStats.in).toLocaleString()}</h4></div></div>
                        <div className="col-6 col-md-4"><div className="card border-0 shadow-sm p-3 bg-danger-subtle h-100"><small className="text-danger fw-bold">TOTAL OUT</small><h4 className="fw-bold mb-0 text-danger">₹{Number(currentStats.out).toLocaleString()}</h4></div></div>
                    </div>
                )}
...
                            <tbody>
                                {loading ? (<tr><td colSpan="7" className="text-center py-5">Loading...</td></tr>) : displayEntries.length > 0 ? (displayEntries.map((entry) => {
                                    const dt = formatEntryTime(entry.entry_date);
                                    return (
                                        <tr key={entry.id}>
                                            <td className="ps-3">
...
                                            </td>
                                            <td><div className="fw-bold text-dark">{entry.account_name || 'General Cash'}</div>{entry.account_mobile && <small className="text-muted"><i className="bi bi-phone"></i> {entry.account_mobile}</small>}</td>
                                            <td>{entry.description || '-'}</td>
                                            <td>
                                                <div className="fw-bold text-dark fst-italic">{entry.created_by || 'Unknown'}</div>
...
                            </tbody>
                            {isLevel1 && (<tfoot className="table-light fw-bold"><tr><td colSpan="5" className="text-end">NET TOTALS:</td><td className={`text-end pe-3 fs-5 ${currentStats.balance >= 0 ? 'text-primary' : 'text-danger'}`}>{currentStats.balance >= 0 ? 'Cr ' : 'Dr '} ₹{Number(Math.abs(currentStats.balance)).toLocaleString()}</td><td className="d-print-none"></td></tr></tfoot>)}
                        </table>
                    </div>
...
                                            <div className="fw-bold fs-5 text-dark text-truncate" style={{maxWidth: '60%'}}>{entry.account_name || 'General Cash'}</div>
                                            <div className="text-end"><small className="text-muted d-block">{dt.date}</small><small className="text-secondary" style={{fontSize: '0.75rem'}}>{dt.time}</small></div>
                                        </div>
                                        <div className="d-flex justify-content-between align-items-center mb-2 small text-secondary">
                                            <span><i className="bi bi-person me-1"></i> {entry.created_by} <span className="ms-1 text-muted" style={{fontSize:'0.7rem'}}>({formatSystemTime(entry.created_at)})</span></span>
                                            <span>{entry.description || '-'}</span>
                                        </div>
                                        <div className="d-flex justify-content-between align-items-center">
                                            <div><span className={`badge ${entry.txn_type === 'IN' ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger'} me-2`}>{entry.txn_type === 'IN' ? 'IN' : 'OUT'}</span><span className={`fw-bold fs-5 ${entry.txn_type === 'IN' ? 'text-success' : 'text-danger'}`}>₹{Number(entry.amount).toLocaleString()}</span></div>
                                            {isLevel1 && <div className="d-flex gap-3">{entry.ledger_account_id && entry.account_mobile && <button onClick={() => handleSendReminder(entry.ledger_account_id)} className="btn btn-link p-0 text-success"><i className="bi bi-whatsapp fs-3"></i></button>}<button onClick={() => handleDeleteEntry(entry.id)} className="btn btn-link p-0 text-danger"><i className="bi bi-trash fs-3"></i></button></div>}
                                        </div>
                                    </div>
                                </div>
                            );
...
                        {isLevel1 && <div className="card border-0 bg-dark text-white text-center p-3 mt-3"><small className="text-white-50 text-uppercase">Net Balance</small><h2 className="fw-bold mb-0"> {currentStats.balance >= 0 ? 'Cr ' : 'Dr '} ₹{Number(Math.abs(currentStats.balance)).toLocaleString()}</h2></div>}
                    </div>
                </div>
            </div>
