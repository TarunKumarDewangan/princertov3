    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            setData(prev => ({
                ...
                total_in: res.data.total_in, total_out: res.data.total_out, balance: res.data.balance
            }));
            setIsFiltered(true);
                                ...
                                <div className="card-body p-3">
                                    ...
                                    <div className="d-flex justify-content-between align-items-center mb-2 border-bottom pb-2">
                                        <div style={{maxWidth: '60%'}}>
                                            <div className="fw-bold fs-5 text-dark text-truncate">{entry.account_name || 'General Cash'}</div>
                                            <small className="text-muted"><i className="bi bi-person"></i> {entry.created_by}</small>
                                        </div>
                                        <div className="text-end">
                                            <small className="text-muted d-block">{new Date(entry.entry_date).toLocaleDateString('en-GB')}</small>
                                            <span className={`badge ${entry.txn_type === 'IN' ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger'}`}>{entry.txn_type}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="d-flex justify-content-between align-items-center">
                                        <span className={`fw-bold fs-4 ${entry.txn_type === 'IN' ? 'text-success' : 'text-danger'}`}>
                                            {entry.txn_type === 'IN' ? '+' : '-'} ₹{Number(entry.amount).toLocaleString()}
                                        </span>

                                        {/* Actions only for Boss */}
                                        {isLevel1 && (
                                            <div className="d-flex gap-3">
                                                {entry.ledger_account_id && entry.account_mobile && <button onClick={() => handleSendReminder(entry.ledger_account_id)} className="btn btn-link p-0 text-success"><i className="bi bi-whatsapp fs-3"></i></button>}
